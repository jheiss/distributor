...
/*
 * Note about local SSL certificates:
 *
 * If your LDAP server is using an SSL certificate that was generated by
 * a local certificate authority (CA) or otherwise wasn't signed by one
 * of the CAs that Java recognizes by default, you'll need to either add
 * you CA cert to Java's list or create your own list.  To go the second
 * route:
 ...
 *
 * Then run Distributor with:
 * java -Djavax.net.ssl.trustStore=<filename> Distributor
 */
...
public class LDAPServiceTest implements Runnable
{
	...
	{
		// This map lists the attributes that must be in the result,
		// and if the map value is non-null the value that that attribute
		// must have.
		Map reqAttrs = new LinkedHashMap();
		reqAttrs.put("objectClass", "posixAccount");
		reqAttrs.put("sn", "Heiss");
		//reqAttrs.put("junk", null);

		LDAPServiceTest lse = new LDAPServiceTest(
			"uid=jheiss,ou=People,dc=aput,dc=net",
			//"uid=foobar,ou=People,dc=aput,dc=net",
			reqAttrs,
			//LDAPServiceTest.SSL_NONE);
			//LDAPServiceTest.SSL_LDAPS);
			LDAPServiceTest.SSL_STARTTLS);
		boolean r = lse.test("ldap1.aput.net", 389);
		//boolean r = lse.test("ldap1.aput.net", 636);
		System.err.println("Test result:  " + r);
	}
	*/

	/*
	 * Options users should be able to give us:
	 * - Use simple auth?
	 *   If so:  username, password
	 * - Or use SASL?
	 *   If so: mechanism, authentication id, auth creds
	 */
	/*
	public LDAPServiceTest(
		String searchDN, Map requiredAttributes, int sslType)
	{
		...
		while (i.hasNext())
		{
			...
			System.err.println("Adding " + attrName + " to result list");
